...
export default function Invoices() {
  ...
  const cycleStatus = async (invoice: Invoice) => {
    const order: InvoiceStatus[] = ['draft', 'sent', 'paid']
    const nextIndex = (order.indexOf(invoice.status) + 1) % order.length
    await supabase.from('invoices').update({ status: order[nextIndex] }).eq('id', invoice.id)
    fetchInvoices()
  }
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {invoices.map((invoice) => (
            <div key={invoice.id} className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-gray-900">{invoice.number}</h3>
                <p className="text-sm text-gray-500">{(invoice.client as unknown as { name: string })?.name} — {(invoice.project as unknown as { name: string })?.name}</p>
                <p className="text-sm text-gray-400 mt-0.5">Vervalt: {new Date(invoice.due_date).toLocaleDateString('nl-NL')}</p>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-lg font-semibold text-gray-900">&euro;{invoice.amount.toFixed(2)}</span>
                <button onClick={() => cycleStatus(invoice)} className={`px-3 py-1 rounded-full text-xs font-medium cursor-pointer ${statusColors[invoice.status]}`}>
                  {statusLabels[invoice.status]}
                </button>
                <button onClick={() => handleDelete(invoice.id)} className="p-2 text-gray-400 hover:text-red-500 transition-colors">
