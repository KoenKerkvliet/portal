...
import { Plus, FolderKanban, Trash2, X, Globe, ExternalLink, ChevronDown, Calendar, Users, Pencil } from 'lucide-react'
...
function InlineEdit({
  ...
  return (
    <button
      ...
      className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 group transition-colors text-left"
...
export default function Projects() {
  ...
  return (
    <div>
      ...
      {/* Create modal — only for new projects */}
      {showForm && (
            ...
            <form onSubmit={handleCreate} className="p-6 space-y-4">
              <div>
                ...
                <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                  ...
                  placeholder="bijv. Website Bakkerij De Gouden Aar" required />
              </div>
              <div>
        ...
        <div className="space-y-4">
          {projects.map((project) => (
            <div key={project.id} className="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow overflow-hidden">
                          ...
                          {(Object.entries(phaseLabels) as [ProjectPhase, string][]).map(([key, label]) => (
                            <button key={key} onClick={() => handlePhaseChange(project, key)}
                              className={`flex items-center gap-2.5 w-full px-3.5 py-2 text-sm transition-colors ${
                                key === project.current_phase ? 'bg-gray-50 font-medium text-gray-900' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                              }`}>
              ...
              {/* Card details — inline editable fields */}
              <div className="px-5 sm:px-6 pb-5 sm:pb-6">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 pt-4 border-t border-gray-100">
                  ...
                  <div className="space-y-1">
                    ...
                    {project.url ? (
                      <div className="flex items-center gap-1.5">
                        <a href={project.url} target="_blank" rel="noopener noreferrer"
                          className="text-sm text-primary hover:text-primary-600 transition-colors truncate">
                    ...
                    <InlineEdit
                      value={project.url || ''}
                      onSave={(url) => updateProject(project.id, { url: url || null })}
                      type="url"
                      placeholder={project.url ? 'Wijzig URL' : 'URL toevoegen'}
                      icon={Globe}
                      displayValue={project.url ? '' : 'URL toevoegen'}
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative" ref={clientDropdownId === project.id ? clientDropdownRef : undefined}>
                      <button
                        onClick={() => setClientDropdownId(clientDropdownId === project.id ? null : project.id)}
                      ...
                      {clientDropdownId === project.id && (
                        <div className="absolute top-full left-0 mt-1.5 bg-white rounded-xl shadow-xl shadow-gray-200/50 border border-gray-100 py-1 z-50 min-w-[200px]">
                          <button
                            onClick={() => handleClientChange(project, '')}
                            className={`flex items-center gap-2.5 w-full px-3.5 py-2 text-sm transition-colors ${
                              !project.client_id ? 'bg-gray-50 font-medium text-gray-900' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                            }`}>
                          ...
                          {clients.map((c) => (
                            <button key={c.id} onClick={() => handleClientChange(project, c.id)}
                              className={`flex items-center gap-2.5 w-full px-3.5 py-2 text-sm transition-colors ${
                                project.client_id === c.id ? 'bg-gray-50 font-medium text-gray-900' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                              }`}>
